...
export type MvpWebPhaseVertical =
  | "broker"
  | "client-experience"
  | "citas-hero"
  | "done";
...
function phaseForFrame(f: number): MvpWebPhaseVertical {
...
function localFrameForPhase(f: number, phase: MvpWebPhaseVertical): number {
  switch (phase) {
    case "broker":
...
/**
 * Mock web MVP 9:16 — misma secuencia que landscape: Tinder → cliente →
 * WhatsApp broker ([MvpWebDoneBrokerScene](MvpWebDoneBrokerScene.tsx)) → titular ([MvpWebCitasHeroScene](MvpWebCitasHeroScene.tsx)).
 */
export function MvpWebMockProVertical({ globalFrame }: Props) {
